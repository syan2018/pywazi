...
*无数渺小的思考填满了人的一生。—— 反正不是我说的*

这份文档不够正经，几乎都是我的胡言乱语还有 AI 的自动补全写出来的，呃呃呃呃，我很抱歉，我没有把这份文档写得正经。
...
当然，但是不是现在，作者需要生活，我会随着心情进行创作。如果你在互联网上收集了一个非常好的资源分享网站，并且期望加入到 PyWazi 中，请提出 Issues 或自己动手并 Pull Request。
...
*很抱歉，对于 waziExHentai.py 我认为优化它的方式就是重构它，我昨天阅读了它，它很糟糕，我很坏。*
...
*我是彩虹色的，我爱你，我是美少女，呃呃呃呃，呃呃呃呃呃呃呃呃呃呃呃呃呃呃呃呃呃呃呃*
...
1. `self.name` 是字符串，储存了当前模块的名字，即 `waziConfig`。

#### readConfig

*焦虑、睡眠不足、寒冷、冻疮，呃呃呃呃呃，我这个早晨过的真是艰辛...*

需要两个参数，一个是 `self` 呃呃呃呃，懂得都懂，一个是 `filePath` 配置文件路径，最后返回字典（如果成功）。

它的逻辑是：打开文件并使用 `json.load` 解析。

### waziFileName

*太过和谐的平五平八，谁愿意写四部和声？我就算了吧...*

使一个不合法的文件名合法，就是这么简单。

#### 相关变量

*我喜欢小二度，呃呃呃呃呃呃呃，好刺激，呃呃呃呃呃呃呃呃，也好伤感，呃呃呃呃呃呃呃呃呃呃呃呃呃呃，呃呃呃呃呃呃呃？*

1. `self.name` 是字符串，储存了当前模块的名字，即 `waziFileName`。

#### toRight

*解决到主和弦上，就是这样。*

提供一个字符串，返回一个字符串（它已经被删除了以下这些不合法字符：\ / : ? " < > |）。

例如：`waziFileName().toRight("w?a.jpg")` -> `wa.jpg`

### waziFun

*Get interval*

获取函数名的函数，是代码最少的一个模块，呃呃呃呃呃呃呃呃额。

#### getFuncName

*故事的开端就 是你和我 —— イントロダクション(仮)前奏*

获取当前函数名，返回字符串。

### waziLog

*不愿意用 logging 模块的孩子们，你们有个好消息和坏消息，好消息是：我们不会再用 logging 模块了，因为我们有了一个日志模块！坏消息是：这个日志模块只是花里胡哨地打印。*

这是一个*不合格*（依谁的标准？）的日志模块，只是打印字符串的模块。但是我不想多花时间处理它，也不想替换 logging 模块，只能说底子没打好之后再改就难了。

#### 相关变量

*只要有色彩和你就足够了...*

1. `self.color` 是 `waziColor` 的实例化，用于色彩生成；
2. `self.min` 表示最小日志等级，默认为 `-1`（即，不打印）；
3. `self.save` 是布尔值，表示是否保存日志，默认为 `False`；
4. `self.saveName` 是字符串，表示日志文件名，系统自动创建。

#### createLogsFolder

*我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，是我们的日志模块，我们的日志模块，*

它被实例化时自动运行，需要一个参数，即 `self`，它将在当前目录下创建 `logs` 文件夹。

#### setSaveName

*一开始的时候什么都没有，为什么最后，什么都有了呢？*

它被实例化时自动运行，需要一个参数，即 `self`，它将按照当前时间生成日志文件名，并将其保存在 `self.saveName` 中。

#### needSave

*一次运行，占用 5MB 大小的日志，你喜欢吗？*

提供两个参数：`self` 和 `boolean`，前者是 `waziLog` 的实例化，后者是布尔值，表示是否保存日志。
...
如果 `self.save` 为 `True`，则将日志内容写入文件；否则，不写入文件。

#### log

*官方媒体*

用于日志打印，你可以在几乎所有的站点模块看到它，它需要三个参数：`self, level, text`，前者是 `waziLog` 的实例化，后者是字符串，可以是 `error`, `warn`, `info`, `debug`；后者是字符串，表示日志内容。

它将调用 `waziColor.print` 和 `waziColor.outputLog`。

### waziRequest

*その函数 その名前 そのコード そのパラメータ*

一个非常*诡异*的网络请求模块，我无法确定*我*一开始创造它的逻辑，因为它臃肿且繁琐，尽管*我*的日志详尽，但我依旧无法理解，这绝对是失眠时做出来的作品。

#### 相关变量

*It is more than I thought could exist —— The xx - Angels*

1. `self.isUseProxies` 是布尔值，表示是否使用代理；
2. `self.proxies` 是字符串，表示代理地址；
3. `self.isUseHeaders` 是布尔值，表示是否使用请求头；
4. `self.headers` 是字符串，表示请求头；
5. `self.name` 是字符串，表示模块名，即 `waziRequest`。

#### useProxies

*整洁得不能再整洁了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求模块又要改变了，我们的网络请求*

它需要两个参数：`self` 和 `isUse`，前者是 `waziRequest` 的实例化，后者是布尔值，表示是否使用代理。完成后将会返回现在代理使用状态。

#### editProxies

*修改代理以保证我的互联网冲浪更加顺畅，现在立刻，非法的逻辑线路 X 1*

它需要三个参数：`self`, `http` 和 `port`，前者是 `waziRequest` 的实例化，后者是字符串，表示代理地址，后者是整数，表示代理端口。完成后将会返回现在代理地址（合成后）。

如果你想要使代理为空，则将 `http` 或 `port` 传入 `None` 即可。

#### useHeaders

*给我的 Bot 办一个互联网冲浪良民证*

它需要两个参数：`self` 和 `isUse`，前者是 `waziRequest` 的实例化，后者是布尔值，表示是否使用自定义请求头部。完成后将会返回现在自定义请求头部状态。

#### editHeaders

*修改自己的名字，在报社登出一则公告，以散发热情*

它需要三个参数：`self`, `key` 和 `value`，前者是 `waziRequest` 的实例化，后者是字符串，表示请求头部键，后者是字符串，表示请求头部值。完成后将会返回现在自定义请求头部。

#### overWriteHeaders

*覆盖自己的身份，就像……转生*

它需要两个参数：`self` 和 `headers`，前者是 `waziRequest` 的实例化，后者是字典，表示自定义请求头部。完成后将会返回现在自定义请求头部。

#### delHeaders

*删除一些不该存在的东西，符合自己完美的设定*

它需要两个参数：`self` 和 `key`，前者是 `waziRequest` 的实例化，后者是字符串，表示请求头部键。完成后将会返回现在自定义请求头部。